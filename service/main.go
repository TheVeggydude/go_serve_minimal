// A simple webserver, built using net/http, which serves two endpoints.
// The first endpoint is a simple health check, which returns a 200 OK.
// The second endpoint is a simple echo, which returns the request body.
package main

import (
	"context"
	"flag"
	"fmt"
	"io"
	"log"
	"net/http"
	"os"
	"os/signal"
	"time"
)

// healthCheck method, which returns a 200 OK
func healthCheck(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, "OK")
}

// echo method, which returns the request body
func echo(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusOK)

	// Read the body into a string for printing later
	body, err := io.ReadAll(r.Body)
	if err != nil {
		log.Println(err)
	}

	io.WriteString(w, string(body))
}

// main method, which starts the webserver and attaches the routes
func main() {

	// Parse command-line flags
	var wait time.Duration
	port := flag.String("port", "8080", "port to listen on")
	flag.DurationVar(&wait, "graceful-timeout", time.Second*15, "the duration for which the server gracefully wait for existing connections to finish - e.g. 15s or 1m")
	flag.Parse()

	// Create a new mux router and attach handlers
	r := http.NewServeMux()
	r.HandleFunc("/health", healthCheck)
	r.HandleFunc("/echo", echo)

	srv := &http.Server{
		Addr: fmt.Sprintf("0.0.0.0:%s", *port),
		// Good practice to set timeouts to avoid Slowloris attacks.
		WriteTimeout: time.Second * 15,
		ReadTimeout:  time.Second * 15,
		IdleTimeout:  time.Second * 60,
		Handler:      r, // Pass our instance of gorilla/mux in.
	}

	// Run our server in a goroutine so that it doesn't block.
	go func() {
		log.Println("listening on port", *port)
		log.Println("graceful shutdown timeout", wait)
		log.Println("starting server...")
		// service connections
		if err := srv.ListenAndServe(); err != nil {
			log.Println(err)
		}
	}()

	c := make(chan os.Signal, 1)
	// We'll accept graceful shutdowns when quit via SIGINT (Ctrl+C)
	// SIGKILL, SIGQUIT or SIGTERM (Ctrl+/) will not be caught.
	signal.Notify(c, os.Interrupt)

	// Block until we receive our signal.
	<-c

	// Create a deadline to wait for.
	ctx, cancel := context.WithTimeout(context.Background(), wait)
	defer cancel()
	// Doesn't block if no connections, but will otherwise wait
	// until the timeout deadline.
	srv.Shutdown(ctx)
	// Optionally, you could run srv.Shutdown in a goroutine and block on
	// <-ctx.Done() if your application should wait for other services
	// to finalize based on context cancellation.
	log.Println("shutting down")
	os.Exit(0)
}
